...
// IDs baseados na sua estrutura de aulas
const MODULOS = [
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/5 pb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium">Sua jornada técnica e autoridade começam aqui.</p>
        </div>
        <div className="bg-[#0A0A0A] border border-[#C9A66B]/30 px-6 py-3 rounded-2xl flex items-center gap-4">
          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Seu Saldo</p>
          <p className="text-xl font-black text-white italic uppercase tracking-tighter">1.100 <span className="text-[#C9A66B]">PRO</span></p>
        </div>
      </div>
        ...
        {MODULOS.map((mod) => (
          <Link key={mod.id} href={`/aula/${mod.id}`} className="group bg-[#0A0A0A] border border-white/5 rounded-3xl overflow-hidden hover:border-[#C9A66B]/30 transition-all">
            <div className="aspect-video bg-zinc-900 flex items-center justify-center relative">
               <div className="absolute top-4 left-4 bg-black/80 px-3 py-1 rounded-lg text-[10px] font-black text-[#C9A66B] uppercase border border-white/10">
                  {mod.id.toUpperCase()}
               </div>
               <PlayCircle size={48} className="text-white/20 group-hover:text-[#C9A66B] transition-colors" />
            </div>
            <div className="p-6">
              <h3 className="font-bold text-white mb-4 leading-tight uppercase tracking-tighter italic">{mod.title}</h3>
              <div className="flex items-center gap-2 text-[#C9A66B] text-[10px] font-black uppercase tracking-widest">
