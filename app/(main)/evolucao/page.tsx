        ...
        {modules?.map((lesson) => (
          ...
          >
            ...
            <div className="aspect-video relative bg-black group-hover:opacity-80 transition-opacity">
                ...
                {/* Ícone Play Outline */}
                <div className="absolute inset-0 flex items-center justify-center z-0 opacity-50 group-hover:opacity-100 transition-opacity">
                     <div className="w-14 h-14 rounded-full border-2 border-white flex items-center justify-center group-hover:border-[#C9A66B] group-hover:scale-110 transition-all">
                        <Play size={24} className="text-white ml-1 group-hover:text-[#C9A66B]" fill="currentColor" />
                     </div>
                </div>
                
                {/* --- TAG DO MÓDULO (OUTLINE) --- */}
                {/* Borda fina, fundo transparente */}
                <div className="absolute top-4 left-4 z-20 border border-[#C9A66B] text-[#C9A66B] text-[10px] font-bold px-3 py-1 rounded-lg uppercase tracking-wider backdrop-blur-sm">
